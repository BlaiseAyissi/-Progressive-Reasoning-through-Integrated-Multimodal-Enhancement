...
class VQA_Slake_Dataset(Dataset):
    def __init__(self, args, tokenizer, close_ended=True, mode="train", knowledge_encoder=False):
        ...
        if mode == "train":
            ...
            with open(os.path.join("/home/cv/Blaise/BaMCo/KSpace/Datasets/Slake1.0", mode + ".json"), 'r') as f:
                self.data_list = json.load(f)
            #self.data_list = pd.read_csv(args.vqa_data_train_path)
        elif mode == "validation":
            with open(os.path.join("/home/cv/Blaise/BaMCo/KSpace/Datasets/Slake1.0", "val" + ".json"), 'r') as f:
                self.data_list = json.load(f)
            #self.data_list = pd.read_csv(args.vqa_data_val_path, nrows=2048)
        elif "test" in mode:
            with open(os.path.join("/home/cv/Blaise/BaMCo/KSpace/Datasets/Slake1.0", mode + ".json"), 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                img_path = os.path.join("/home/cv/Blaise/BaMCo/KSpace/Datasets/Slake1.0/imgs", self.data_list[idx]["img_name"])
